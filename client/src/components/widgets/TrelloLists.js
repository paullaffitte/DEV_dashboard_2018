...
class TrelloLists extends Component {
  ...
  update = async () => {
    this.setState({
      data: (await this.client.getListsOnBoard(this.props.config.boardId)).map((e, idx) => ({...e, key: idx}))      // data: (await this.client.getCardsOnList(this.props.config.listId)).map((e, idx) => ({...e, key: idx}))
    });
    const board = await this.client.makeRequest('get', `/1/boards/${this.props.config.boardId}`);
    this.props.onTitleUpdate(board.name + ' Lists - Trello');
  }
...
class __TrelloListsForm extends Component {
  render() {
...
const TrelloListsForm = Form.create()(__TrelloListsForm);
